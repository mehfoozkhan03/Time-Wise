...
export default function Performance() {
  ...
  const weeklySubtitle = () => {
    ...
    if (stats.weeklyHoursRemaining <= 0) {
      ...
      const extra =
        Math.abs(stats.weeklyHoursRemaining).toFixed(1);
  ...
  const streakSubtitle = () => {
    ...
    if (
      stats.longestStreak > 0 &&
      stats.dayStreak === stats.longestStreak
    ) {
      return '🏆 New personal best!';
    }
  ...
  const weeklyColor = () => {
    ...
    if (stats.weeklyGoalPercentage >= 100)
      return '#22c55e';

    if (stats.weeklyGoalPercentage >= 80)
      return '#8b5cf6';
