  ...
  return (
    <div className="dashboardSetting-container">
      ...
      <section className="dashboard-ai-setting simple-ai-setting">
        {isLoading ? <p className="ai-settings-message">Loading WiseBot access...</p> : (
          <>
            <div className="dashboard-ai-setting-header">
              <div>
                <h3>WiseBot</h3>
                <p>{isEnabled ? "Available to everyone." : settings?.accessMode === "disabled_for_everyone" ? "Disabled for the full organisation." : "Disabled for selected employees."}</p>
              </div>
              <label className="ai-toggle">
                <input type="checkbox" checked={isEnabled} disabled={isSaving} onChange={(event) => event.target.checked ? save("enabled") : setShowDisableChoice(true)} />
            ...
            {showDisableChoice && (
              <div className="ai-disable-choice">
                ...
                <label><input type="radio" name="disable-mode" checked={mode === "disabled_for_everyone"} onChange={() => setMode("disabled_for_everyone")} /> Full organisation</label>
                <label><input type="radio" name="disable-mode" checked={mode === "disabled_for_selected"} onChange={() => setMode("disabled_for_selected")} /> Selected employees</label>
                  ...
                  <div className="ai-employee-list">
                    {employees.map((employee) => (
                      <label key={employee._id}>
                        ...
                        <span>{employee.firstName} {employee.lastName}<small>{employee.email}</small></span>
                      </label>
                    ))}
                  </div>
                )}
                <div className="ai-choice-actions">
                  <button type="button" onClick={() => setShowDisableChoice(false)} disabled={isSaving}>Cancel</button>
                  <button type="button" onClick={() => save(mode, mode === "disabled_for_selected" ? selectedIDs : [])} disabled={isSaving || (mode === "disabled_for_selected" && selectedIDs.length === 0)}>{isSaving ? "Saving..." : "Confirm disable"}</button>
                </div>
              </div>
            )}
